...
router.get('/:slug', async (req, res) => {
  try {
    ...
    if (phoneQuery) {
      ...
      if (match.length === 0) {
        return res.status(404).json({ message: 'No student found with this phone number. Use only your own registered number. Do not use another student\'s number. One click only—no second attempt with a different number.' });
      }
      if (match.length > 1) {
