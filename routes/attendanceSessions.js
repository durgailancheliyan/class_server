...
router.get('/:slug', async (req, res) => {
  try {
    ...
    if (phoneQuery) {
      ...
      if (match.length === 0) {
        return res.status(404).json({ message: 'No student found with this phone number in this batch. Only a registered student phone number can view details and mark attendance.' });
      }
      if (match.length > 1) {
